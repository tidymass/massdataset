...
![GitHub page for creating a personal access token.](../man/figures/Screen Shot 2021-12-24 at 9.28.18 PM.png)
...
`raster` is available from [CRAN](https://CRAN.R-project.org/package=raster), so try installing it first and then install `massdataset`.
...
`Cairo` is available from [CRAN](https://CRAN.R-project.org/package=Cairo), so try installing it first and then install `massdataset`.
