...
test_that(desc = "mutate_sample_na_number",
          code = {
            object1 <- 
              activate_mass_dataset(object, what = "expression_data")
            object1 <-
              mutate_sample_na_number(object = object1, 
                                      according_to_variables = object1@variable_info$variable_id)
            result1 <-
              check_mass_dataset(
...
test_that(desc = "mutate_sample_na_freq",
          code = {
            object1 <- 
              activate_mass_dataset(object, what = "expression_data")
            object1 <-
              mutate_sample_na_freq(object = object1, 
                                      according_to_variables = object1@variable_info$variable_id)
            result1 <-
              check_mass_dataset(
                expression_data = object1@expression_data,
