...
#'
#' This function lists all the conflicts between packages in the massdataset
...
massdataset_conflicts <- function() {
  ...
  tidy_names <- paste0("package:", massdataset_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))
  
  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)
  
  structure(conflict_funs, class = "massdataset_conflicts")
}

massdataset_conflict_message <- function(x) {
  if (length(x) == 0) return("")
  
  header <- cli::rule(
    left = crayon::bold("Conflicts"),
    right = "massdataset_conflicts()"
  )
...
#' @export
print.massdataset_conflicts <- function(x, ..., startup = FALSE) {
